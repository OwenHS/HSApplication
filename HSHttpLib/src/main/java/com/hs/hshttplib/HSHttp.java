...
//	private I_FileLoader downloader;
	...
	public void post(String url, JSONObject object, HttpCallBack callback)
	{
		VollyTask.setDefaultExecutor(VollyTask.mLruSerialExecutor);
		if (object != null)
		{
			new VollyTask(Method.POSTJSON, url, null, object, callback).execute();
		}
		else
		{
			new VollyTask(Method.GET, url, null, null, callback).execute();
	...
	public void download(String url, File save, HttpCallBack callback)
	{
		...
		download(url, save, new SimpleDownloader(httpConfig, new FileProxyHttpCallBack(url, callback, new FileFinishCallBack()
		{
	 ...
	 *            下载器
	 */
	private void download(String url, File save, I_FileLoader downloader)
	{
		try
		{
			if (!save.exists())
	...
	public class VollyTask extends HSCachedTask<String, Object, String>
	{
		...
		public VollyTask(Method method, String uri, HttpParams params, JSONObject object, HttpCallBack callBack)
		{
			...
			String cookie = /* httpConfig.httpHeader.get("Cookie") */null;
		...
		protected void onPostExecuteSafely(String result, Exception e) throws Exception
		{
			...
			{
				callBack.onFailure(e, responseCode, responseMsg);
			}
			callBack.onFinish();
		}
	}
			...
			String charsetName) throws IOException
	{
		...
		case POST:
			...
			// 设置容许输入,输出到流中,post需要提交参数
			connection.setDoOutput(true);
			connection.setInstanceFollowRedirects(true);
			connection.setRequestProperty("connection", "Keep-Alive");
			connection.setRequestProperty("Content-Type", "multipart/form-data; boundary=" + HttpConfig.BOUNDARY);
			break;
		case POSTJSON:
			connection.setRequestMethod("POST");
			// 设置容许输入,输出到流中,post需要提交参数
		...
		if (requestMethod == Method.POST && params != null)
		{
			...
			try
			{
				...
				{
					sb.append("--").append(HttpConfig.BOUNDARY).append("\r\nContent-Disposition: form-data;name=\"")
							.append(entry.getKey()).append("\";filename=\"").append(entry.getValue().fileName)
		...
		if (requestMethod == Method.POSTJSON && object != null)
		{
			DataOutputStream out = null;
			try
			{
				out = new DataOutputStream(connection.getOutputStream());
				out.write(object.toString().getBytes(charsetName));
				out.flush();
			}
			finally
			{
